...
namespace NAudio.Flac
{
	public static class FrameIDFactory2
	{
		public class ID3v2FrameEntry
		{
			...
			public string ID3v4ID
			{
				get;
				set;
			}

			public string ID3v3ID
			{
				get;
				set;
			}

			public string ID3v2ID
			{
				get;
				set;
			}
		...
		public static ID3v2FrameEntry GetFrameEntry(string id, ID3Version version)
		{
			...
			case ID3Version.ID3v2_2:
				return Frames.Where((ID3v2FrameEntry x) => !string.IsNullOrEmpty(x.ID3v2ID) && x.ID3v2ID.Equals(id, StringComparison.InvariantCultureIgnoreCase)).First();
			case ID3Version.ID3v2_3:
				return Frames.Where((ID3v2FrameEntry x) => !string.IsNullOrEmpty(x.ID3v3ID) && x.ID3v3ID.Equals(id, StringComparison.InvariantCultureIgnoreCase)).First();
			case ID3Version.ID3v2_4:
				return Frames.Where((ID3v2FrameEntry x) => !string.IsNullOrEmpty(x.ID3v4ID) && x.ID3v4ID.Equals(id, StringComparison.InvariantCultureIgnoreCase)).First();
			default:
		...
		{
			ID3v2FrameEntry item = new ID3v2FrameEntry
			{
		...
		{
			IEnumerable<ID3v2FrameEntry> enumerable = Frames.Where((ID3v2FrameEntry x) => x.ID == id);
			if (enumerable != null && enumerable.Count() > 0)
			{
				ID3v2FrameEntry iD3v2FrameEntry = enumerable.First();
				string text = null;
				switch (version)
				{
				case ID3Version.ID3v2_2:
					text = iD3v2FrameEntry.ID3v2ID;
					break;
				case ID3Version.ID3v2_3:
					text = iD3v2FrameEntry.ID3v3ID;
					break;
				case ID3Version.ID3v2_4:
					text = iD3v2FrameEntry.ID3v4ID;
					break;
				default:
